...
export default function CheckoutPage() {
  ...
  const handleSimulatePayment = async () => {
    setProcessing(true);
    // Simulate gateway delay (BAC/Serfinsa redirect simulation)
            ...
            {paymentMethod === 'qr' && (
              <div className="bg-white p-6 rounded-3xl flex flex-col items-center space-y-4 animate-in zoom-in duration-500">
                <div className="w-48 h-48 bg-zinc-100 rounded-2xl flex items-center justify-center border-4 border-zinc-50">
                  <img 
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=ready2drive:${reservationId}`} 
                    alt="QuickPay QR" 
              ...
              <CardContent className="p-8 space-y-8">
                ...
                <div className="space-y-4 py-6 border-y border-zinc-800">
                  <div className="flex justify-between text-zinc-400">
                    ...
                    <span className="text-white font-bold">${(reservation?.total_price / 1.05).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-zinc-400">
                    <span className="italic">Comisión Bancaria (5%)</span>
                    <span className="text-white font-bold">${(reservation?.total_price - (reservation?.total_price / 1.05)).toFixed(2)}</span>
                  </div>
                </div>
